...
for (Int_t i = 0; i<nentries; i++) 
{
  ...
  */
  ...
  { 
    ...
    {   // muons
      muon_num++;
      Muon_pt[bb]=(lep_pt->at(bb)); // MeV to GeV conversion | ATLAS to CMS //remove conversions as already in GeV //edit
    ...
    //  Muon_e[bb]=(lep_E->at(bb)/1000.);  // This can be calculated from pt and eta and mass so its not actually there in nanoAODv8, will be removed later. 
     ...
     //dummy variables not dependent on ATLAS data
      ...
      Muon_isGlobal[bb]=true;   // Quality cuts already applied by ATLAS, so set to true by default. 
      Muon_sip3d[bb]=0.0;  
      /*************************************************************/
    ...
    {   // Electrons
      ele_num++;
      Electron_pt[bb]=(lep_pt->at(bb)); // MeV to GeV conversion | ATLAS to CMS //remove conversions as already in GeV //edit
...
void atlas2016_to_cms_nanoAODplus_revived_adapted() {
  ...
  atlastocmsfile("/afs/desy.de/user/l/lukerlou/OpenData/data/submitDir-2025-07-15-1059-ad24/data-ANALYSIS/output.root","/afs/desy.de/user/l/lukerlou/OpenData/data/submitDir-2025-07-15-1059-ad24/data-ANALYSIS/output_cms.root",2016); //edit
